...
/*
* File:   usfm_Projection_Division2.hpp
* Author: Michal Polic, michal.polic(at)cvut.cz
*/

#ifndef USFM_PROJECTION_DIVISION2_H
#define USFM_PROJECTION_DIVISION2_H
...
namespace usfm {
	...
	class ProjectionDivision2 : public Projection {
	public:
		static const int N_CAM_PARAMS = 5;
		static const int N_IMG_PARAMS = 6;

		ProjectionDivision2();
		ProjectionDivision2(Camera* camera);
		ProjectionDivision2(Image* camera);
		ProjectionDivision2(Camera* camera, Image* image);
		ProjectionDivision2(Camera* camera, Image* image, Point3D* X, Point2D* obs);
		...
		bool operator()(const T* const X, const T* const cam, const T* const img, T* residuals) const 
		{
			...
			T dist = x*x + y*y;
			T distortion = T(1.0) + k[0] * dist + k[1] * dist * dist;
...
#endif //USFM_PROJECTION_DIVISION2_H