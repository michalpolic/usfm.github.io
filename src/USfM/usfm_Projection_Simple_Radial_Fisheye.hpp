...
/*
* File:   usfm_Projection_Simple_Radial_Fisheye.hpp
* Author: Michal Polic, michal.polic(at)cvut.cz
*/

#ifndef USFM_PROJECTION_SIMPLE_RADIAL_FISHEYE_H
#define USFM_PROJECTION_SIMPLE_RADIAL_FISHEYE_H
...
namespace usfm {
	...
	class ProjectionSimpleRadialFisheye : public Projection {
	public:
		static const int N_CAM_PARAMS = 4;
		static const int N_IMG_PARAMS = 6;

		ProjectionSimpleRadialFisheye();
		ProjectionSimpleRadialFisheye(Camera* camera);
		ProjectionSimpleRadialFisheye(Image* camera);
		ProjectionSimpleRadialFisheye(Camera* camera, Image* image);
		ProjectionSimpleRadialFisheye(Camera* camera, Image* image, Point3D* X, Point2D* obs);
		...
		{
			// the internal/external parameters			f, cx, cy, k
...
#endif //USFM_PROJECTION_SIMPLE_RADIAL_FISHEYE_H