...
/*
* File:   usfm_Projection_Radial3Division3.hpp
* Author: Michal Polic, michal.polic(at)cvut.cz
*/

#ifndef USFM_PROJECTION_RADIAL3DIVISION3_H
#define USFM_PROJECTION_RADIAL3DIVISION3_H
...
namespace usfm {
	...
	class ProjectionRadial3Division3 : public ProjectionACC {
	public:
		static const int N_CAM_PARAMS = 9;

		ProjectionRadial3Division3();
		ProjectionRadial3Division3(Camera* camera);
		ProjectionRadial3Division3(Image* camera);
		ProjectionRadial3Division3(Camera* camera, Image* image);
		ProjectionRadial3Division3(Camera* camera, Image* image, Point3D* X, Point2D* obs);
		...
		bool operator()(const T* const X, const T* const cam, const T* const img, T* residuals) const
		{
			...
			const T k[6] = { cam[3], cam[4], cam[5], cam[6], cam[7], cam[8] };		// radial distortion parameters
			...
			const T r4 = r2 * r2;
			const T r6 = r4 * r2;
			const T distortion = (T(1.0) + k[0] * r2 + k[1] * r4 + k[2] * r6) / (T(1.0) + k[3] * r2 + k[4] * r4 + k[5] * r6);
...
#endif //USFM_PROJECTION_RADIAL3DIVISION3_H