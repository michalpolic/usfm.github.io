...
/*
* File:   usfm_Projection_Division1.cpp
...
namespace usfm {
	...
	ProjectionDivision1::ProjectionDivision1() {}
	ProjectionDivision1::ProjectionDivision1(Image* image) {
		_image = image;
	};
	ProjectionDivision1::ProjectionDivision1(Camera* camera) {
		_camera = camera;
	};
	ProjectionDivision1::ProjectionDivision1(Camera* camera, Image* image) {
		_camera = camera;
		_image = image;
	}
	ProjectionDivision1::ProjectionDivision1(Camera* camera, Image* image, Point3D* X, Point2D* obs) {
		_camera = camera;
		_image = image;
		_X = X;
		_obs = obs;
	...
	void ProjectionDivision1::initOffsets(Camera* cam) {
	...
	void ProjectionDivision1::reorderParams(Camera* cam) {
		if (!cam)
	...
	void ProjectionDivision1::initOffsets(Image* img) {
	...
	void ProjectionDivision1::reorderParams(Image* img) {
	...
	ceres::CostFunction* ProjectionDivision1::getCostFunction() {
		return (new ceres::AutoDiffCostFunction<ProjectionDivision1,
			2,												// output residuals
			3,												// input 3D point	
			ProjectionDivision1::N_CAM_PARAMS,			// camera parameters
			ProjectionDivision1::N_IMG_PARAMS>(new ProjectionDivision1(_camera, _image, _X, _obs)));	// image parameters
	}

	// compute the residual of related 3D point projection and measured observation
	void ProjectionDivision1::computeResidual(double * res) {
	...
	int ProjectionDivision1::numCamParams() {
		return ProjectionDivision1::N_CAM_PARAMS;
	}

	// number of used image parameters for specified image model
	int ProjectionDivision1::numImgParams() {
		return ProjectionDivision1::N_IMG_PARAMS;
	}
}