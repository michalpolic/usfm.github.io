...
/*
* File:   usfm_Projection_Radial2.cpp
...
namespace usfm {
	...
	ProjectionRadial2::ProjectionRadial2() {}

	ProjectionRadial2::ProjectionRadial2(Image* image) {
		_image = image;
	};

	ProjectionRadial2::ProjectionRadial2(Camera* camera) {
		_camera = camera;
	};

	ProjectionRadial2::ProjectionRadial2(Camera* camera, Image* image) {
		_camera = camera;
		_image = image;
	}

	ProjectionRadial2::ProjectionRadial2(Camera* camera, Image* image, Point3D* X, Point2D* obs) {
		_camera = camera;
		_image = image;
		_X = X;
		_obs = obs;
	...
	void ProjectionRadial2::initOffsets(Camera* cam) {
	...
	void ProjectionRadial2::reorderParams(Camera* cam) {
		if (!cam)
	...
	void ProjectionRadial2::initOffsets(Image* img) {
	...
	void ProjectionRadial2::reorderParams(Image* img) {
	...
	ceres::CostFunction* ProjectionRadial2::getCostFunction() {
		return (new ceres::AutoDiffCostFunction<ProjectionRadial2,
			2,												// output residuals
			3,												// input 3D point	
			ProjectionRadial2::N_CAM_PARAMS,					// camera parameters
			ProjectionRadial2::N_IMG_PARAMS>(new ProjectionRadial2(_camera,_image,_X,_obs)));			// image parameters
	}

	// compute the residual of related 3D point projection and measured observation
	void ProjectionRadial2::computeResidual(double * res){
	...
	int ProjectionRadial2::numCamParams() {
		return ProjectionRadial2::N_CAM_PARAMS;
	}

	// number of used image parameters for specified image model
	int ProjectionRadial2::numImgParams() {
		return ProjectionRadial2::N_IMG_PARAMS;
	}
