...
/*
* File:   usfm_Projection_Division4.cpp
...
namespace usfm {
	...
	ProjectionDivision4::ProjectionDivision4() {}
	ProjectionDivision4::ProjectionDivision4(Image* image) {
		_image = image;
	};
	ProjectionDivision4::ProjectionDivision4(Camera* camera) {
		_camera = camera;
	};
	ProjectionDivision4::ProjectionDivision4(Camera* camera, Image* image) {
		_camera = camera;
		_image = image;
	}
	ProjectionDivision4::ProjectionDivision4(Camera* camera, Image* image, Point3D* X, Point2D* obs) {
		_camera = camera;
		_image = image;
		_X = X;
		_obs = obs;
	...
	void ProjectionDivision4::initOffsets(Camera* cam) {
	...
	void ProjectionDivision4::reorderParams(Camera* cam) {
		if (!cam)
	...
	void ProjectionDivision4::initOffsets(Image* img) {
	...
	void ProjectionDivision4::reorderParams(Image* img) {
	...
	ceres::CostFunction* ProjectionDivision4::getCostFunction() {
		return (new ceres::AutoDiffCostFunction<ProjectionDivision4,
			2,												// output residuals
			3,												// input 3D point	
			ProjectionDivision4::N_CAM_PARAMS,			// camera parameters
			ProjectionDivision4::N_IMG_PARAMS>(new ProjectionDivision4(_camera, _image, _X, _obs)));	// image parameters
	}

	// compute the residual of related 3D point projection and measured observation
	void ProjectionDivision4::computeResidual(double * res) {
	...
	int ProjectionDivision4::numCamParams() {
		return ProjectionDivision4::N_CAM_PARAMS;
	}

	// number of used image parameters for specified image model
	int ProjectionDivision4::numImgParams() {
		return ProjectionDivision4::N_IMG_PARAMS;
	}
}