...
/*
* File:   usfm_Projection_Radial1.cpp
...
namespace usfm {
	...
	ProjectionRadial1::ProjectionRadial1() {}
	ProjectionRadial1::ProjectionRadial1(Image* image) {
		_image = image;
	};
	ProjectionRadial1::ProjectionRadial1(Camera* camera) {
		_camera = camera;
	};
	ProjectionRadial1::ProjectionRadial1(Camera* camera, Image* image) {
		_camera = camera;
		_image = image;
	}
	ProjectionRadial1::ProjectionRadial1(Camera* camera, Image* image, Point3D* X, Point2D* obs) {
		_camera = camera;
		_image = image;
		_X = X;
		_obs = obs;
	...
	void ProjectionRadial1::initOffsets(Camera* cam) {
	...
	void ProjectionRadial1::reorderParams(Camera* cam) {
		if (!cam)
	...
	void ProjectionRadial1::initOffsets(Image* img) {
	...
	void ProjectionRadial1::reorderParams(Image* img) {
	...
	ceres::CostFunction* ProjectionRadial1::getCostFunction() {
		return (new ceres::AutoDiffCostFunction<ProjectionRadial1,
			2,												// output residuals
			3,												// input 3D point	
			ProjectionRadial1::N_CAM_PARAMS,			// camera parameters
			ProjectionRadial1::N_IMG_PARAMS>(new ProjectionRadial1(_camera, _image, _X, _obs)));	// image parameters
	}

	// compute the residual of related 3D point projection and measured observation
	void ProjectionRadial1::computeResidual(double * res) {
	...
	int ProjectionRadial1::numCamParams() {
		return ProjectionRadial1::N_CAM_PARAMS;
	}

	// number of used image parameters for specified image model
	int ProjectionRadial1::numImgParams() {
		return ProjectionRadial1::N_IMG_PARAMS;
	}
}