...
/*
* File:   usfm_Projection_Radial_Fisheye.cpp
...
namespace usfm {
	...
	ProjectionRadialFisheye::ProjectionRadialFisheye() {}
	ProjectionRadialFisheye::ProjectionRadialFisheye(Image* image) {
		_image = image;
	};
	ProjectionRadialFisheye::ProjectionRadialFisheye(Camera* camera) {
		_camera = camera;
	};
	ProjectionRadialFisheye::ProjectionRadialFisheye(Camera* camera, Image* image) {
		_camera = camera;
		_image = image;
	}
	ProjectionRadialFisheye::ProjectionRadialFisheye(Camera* camera, Image* image, Point3D* X, Point2D* obs) {
		_camera = camera;
		_image = image;
		_X = X;
		_obs = obs;
	...
	void ProjectionRadialFisheye::initOffsets(Camera* cam) {
	...
	void ProjectionRadialFisheye::reorderParams(Camera* cam) {
		if (!cam)
	...
	void ProjectionRadialFisheye::initOffsets(Image* img) {
	...
	void ProjectionRadialFisheye::reorderParams(Image* img) {
	...
	ceres::CostFunction* ProjectionRadialFisheye::getCostFunction() {
		return (new ceres::AutoDiffCostFunction<ProjectionRadialFisheye,
			2,												// output residuals
			3,												// input 3D point	
			ProjectionRadialFisheye::N_CAM_PARAMS,			// camera parameters
			ProjectionRadialFisheye::N_IMG_PARAMS>(new ProjectionRadialFisheye(_camera, _image, _X, _obs)));	// image parameters
	}

	// compute the residual of related 3D point projection and measured observation
	void ProjectionRadialFisheye::computeResidual(double * res) {
	...
	int ProjectionRadialFisheye::numCamParams() {
		return ProjectionRadialFisheye::N_CAM_PARAMS;
	}

	// number of used image parameters for specified image model
	int ProjectionRadialFisheye::numImgParams() {
		return ProjectionRadialFisheye::N_IMG_PARAMS;
	}
}