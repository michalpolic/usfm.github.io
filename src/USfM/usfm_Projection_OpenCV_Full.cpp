...
/*
* File:   usfm_Projection_OpenCV_Full.cpp
...
namespace usfm {
	...
	ProjectionOpenCVFull::ProjectionOpenCVFull() {}
	ProjectionOpenCVFull::ProjectionOpenCVFull(Image* image) {
		_image = image;
	};
	ProjectionOpenCVFull::ProjectionOpenCVFull(Camera* camera) {
		_camera = camera;
	};
	ProjectionOpenCVFull::ProjectionOpenCVFull(Camera* camera, Image* image) {
		_camera = camera;
		_image = image;
	}
	ProjectionOpenCVFull::ProjectionOpenCVFull(Camera* camera, Image* image, Point3D* X, Point2D* obs) {
		_camera = camera;
		_image = image;
		_X = X;
		_obs = obs;
	...
	void ProjectionOpenCVFull::initOffsets(Camera* cam) {		//fx, fy, cx, cy, k1, k2, p1, p2, k3, k4, k5, k6
		...
		cam->_offset_in_parameters[e_k3] = 10;
		cam->_offset_in_parameters[e_k4] = 11;
		cam->_offset_in_parameters[e_k5] = 12;
		cam->_offset_in_parameters[e_k6] = 13;
	}

	// sort the parameters array (allows the change of projection function)
	void ProjectionOpenCVFull::reorderParams(Camera* cam) {
		if (!cam)
		...
		exchangeParameters(cam, 8, e_k3);
		exchangeParameters(cam, 9, e_k4);
		exchangeParameters(cam, 10, e_k5);
		exchangeParameters(cam, 11, e_k6);
	}

	// init pointers to images
	void ProjectionOpenCVFull::initOffsets(Image* img) {
	...
	void ProjectionOpenCVFull::reorderParams(Image* img) {
	...
	ceres::CostFunction* ProjectionOpenCVFull::getCostFunction() {
		return (new ceres::AutoDiffCostFunction<ProjectionOpenCVFull,
			2,												// output residuals
			3,												// input 3D point	
			ProjectionOpenCVFull::N_CAM_PARAMS,					// camera parameters
			ProjectionOpenCVFull::N_IMG_PARAMS>(new ProjectionOpenCVFull(_camera, _image, _X, _obs)));			// image parameters
	}

	// compute the residual of related 3D point projection and measured observation
	void ProjectionOpenCVFull::computeResidual(double * res) {
	...
	int ProjectionOpenCVFull::numCamParams() {
		return ProjectionOpenCVFull::N_CAM_PARAMS;
	}

	// number of used image parameters for specified image model
	int ProjectionOpenCVFull::numImgParams() {
		return ProjectionOpenCVFull::N_IMG_PARAMS;
	}
