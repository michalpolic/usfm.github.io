...
/*
* File:   usfm_Projection_Radial3Division1.hpp
* Author: Michal Polic, michal.polic(at)cvut.cz
*/

#ifndef USFM_PROJECTION_RADIAL3DIVISION1_H
#define USFM_PROJECTION_RADIAL3DIVISION1_H
...
namespace usfm {
	...
	class ProjectionRadial3Division1 : public ProjectionACC {
	public:
		static const int N_CAM_PARAMS = 7;

		ProjectionRadial3Division1();
		ProjectionRadial3Division1(Camera* camera);
		ProjectionRadial3Division1(Image* camera);
		ProjectionRadial3Division1(Camera* camera, Image* image);
		ProjectionRadial3Division1(Camera* camera, Image* image, Point3D* X, Point2D* obs);
		...
		bool operator()(const T* const X, const T* const cam, const T* const img, T* residuals) const
		{
			...
			const T k[4] = { cam[3], cam[4], cam[5], cam[6] };	// radial distortion parameters
			...
			const T r4 = r2 * r2;
			const T r6 = r4 * r2;
			const T distortion = (T(1.0) + k[0] * r2 + k[1] * r4 + k[2] * r6) / (T(1.0) + k[3] * r2 );
...
#endif //USFM_PROJECTION_RADIAL3DIVISION1_H