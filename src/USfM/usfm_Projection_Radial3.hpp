...
/*
* File:   usfm_Projection_Radial3.hpp
* Author: Michal Polic, michal.polic(at)cvut.cz
*/

#ifndef USFM_PROJECTION_RADIAL3_H
#define USFM_PROJECTION_RADIAL3_H
...
namespace usfm {
	...
	class ProjectionRadial3 : public Projection {
	public:
		static const int N_CAM_PARAMS = 6;
		static const int N_IMG_PARAMS = 6;

		ProjectionRadial3();
		ProjectionRadial3(Camera* camera);
		ProjectionRadial3(Image* camera);
		ProjectionRadial3(Camera* camera, Image* image);
		ProjectionRadial3(Camera* camera, Image* image, Point3D* X, Point2D* obs);
		...
		{
			// the internal/external parameters			f, cx, cy, k1, k2, k3
			T f = cam[0];								// focal length
			T pp[2] = { cam[1], cam[2] };				// principal point
			T k[3] = { cam[3], cam[4], cam[5] };		// radial distortion parameters
			...
			T dist = x*x + y*y;
			T distortion = T(1.0) + k[0]*dist + k[1]*dist*dist + k[2]*dist*dist*dist;
...
#endif //USFM_PROJECTION_RADIAL3_H