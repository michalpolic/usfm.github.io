...
/*
* File:   usfm_Projection_Division4.hpp
* Author: Michal Polic, michal.polic(at)cvut.cz
*/

#ifndef USFM_PROJECTION_DIVISION4_H
#define USFM_PROJECTION_DIVISION4_H
...
namespace usfm {
	...
	class ProjectionDivision4 : public Projection {
	public:
		static const int N_CAM_PARAMS = 7;
		static const int N_IMG_PARAMS = 6;

		ProjectionDivision4();
		ProjectionDivision4(Camera* camera);
		ProjectionDivision4(Image* camera);
		ProjectionDivision4(Camera* camera, Image* image);
		ProjectionDivision4(Camera* camera, Image* image, Point3D* X, Point2D* obs);
		...
		bool operator()(const T* const X, const T* const cam, const T* const img, T* residuals) const 
		{
			...
			T dist2 = x*x + y*y;
			T dist4 = dist2 * dist2;
			T distortion = T(1.0) + k[0] * dist2 + k[1] * dist4 + k[2] * dist4 * dist2 + k[3] * dist4 * dist4;
...
#endif //USFM_PROJECTION_DIVISION4_H