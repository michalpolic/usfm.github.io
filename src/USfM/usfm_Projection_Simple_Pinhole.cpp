...
/*
* File:   usfm_Projection_Simple_Pinhole.cpp
...
namespace usfm {
	...
	ProjectionSimplePinhole::ProjectionSimplePinhole() {}

	ProjectionSimplePinhole::ProjectionSimplePinhole(Image* image) {
		_image = image;
	};

	ProjectionSimplePinhole::ProjectionSimplePinhole(Camera* camera) {
		_camera = camera;
	};

	ProjectionSimplePinhole::ProjectionSimplePinhole(Camera* camera, Image* image) {
		_camera = camera;
		_image = image;
	}

	ProjectionSimplePinhole::ProjectionSimplePinhole(Camera* camera, Image* image, Point3D* X, Point2D* obs) {
		_camera = camera;
		_image = image;
		_X = X;
		_obs = obs;
	...
	void ProjectionSimplePinhole::initOffsets(Camera* cam) {
	...
	void ProjectionSimplePinhole::reorderParams(Camera* cam) {
		if (!cam)
	...
	void ProjectionSimplePinhole::initOffsets(Image* img) {
	...
	void ProjectionSimplePinhole::reorderParams(Image* img) {
	...
	ceres::CostFunction* ProjectionSimplePinhole::getCostFunction() {
		return (new ceres::AutoDiffCostFunction<ProjectionSimplePinhole,
			2,												// output residuals
			3,												// input 3D point	
			ProjectionSimplePinhole::N_CAM_PARAMS,			// camera parameters
			ProjectionSimplePinhole::N_IMG_PARAMS>(new ProjectionSimplePinhole(_camera, _image, _X, _obs)));	// image parameters
	}

	// compute the residual of related 3D point projection and measured observation
	void ProjectionSimplePinhole::computeResidual(double * res)	{
	...
	int ProjectionSimplePinhole::numCamParams() {
		return ProjectionSimplePinhole::N_CAM_PARAMS;
	}

	// number of used image parameters for specified image model
	int ProjectionSimplePinhole::numImgParams() {
		return ProjectionSimplePinhole::N_IMG_PARAMS;
	}
