...
/*
* File:   usfm_Projection_Pinhole.hpp
* Author: Michal Polic, michal.polic(at)cvut.cz
*/

#ifndef USFM_PROJECTION_PINHOLE_H
#define USFM_PROJECTION_PINHOLE_H
...
namespace usfm {
	...
	class ProjectionPinhole : public Projection {
	public:
		static const int N_CAM_PARAMS = 4;
		static const int N_IMG_PARAMS = 6;

		ProjectionPinhole();
		ProjectionPinhole(Camera* camera);
		ProjectionPinhole(Image* camera);
		ProjectionPinhole(Camera* camera, Image* image);
		ProjectionPinhole(Camera* camera, Image* image, Point3D* X, Point2D* obs);
		...
		bool operator()(const T* const X, const T* const cam, const T* const img, T* residuals) const
		{
			...
			T f[2] = { cam[0], cam[1] };				// focal length
			T pp[2] = { cam[2], cam[3] };				// principal point
