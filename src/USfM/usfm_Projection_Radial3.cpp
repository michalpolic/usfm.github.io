...
/*
* File:   usfm_Projection_Radial3.cpp
...
namespace usfm {
	...
	ProjectionRadial3::ProjectionRadial3() {}
	ProjectionRadial3::ProjectionRadial3(Image* image) {
		_image = image;
	};
	ProjectionRadial3::ProjectionRadial3(Camera* camera) {
		_camera = camera;
	};
	ProjectionRadial3::ProjectionRadial3(Camera* camera, Image* image) {
		_camera = camera;
		_image = image;
	}
	ProjectionRadial3::ProjectionRadial3(Camera* camera, Image* image, Point3D* X, Point2D* obs) {
		_camera = camera;
		_image = image;
		_X = X;
		_obs = obs;
	...
	void ProjectionRadial3::initOffsets(Camera* cam) {
	...
	void ProjectionRadial3::reorderParams(Camera* cam) {
		if (!cam)
	...
	void ProjectionRadial3::initOffsets(Image* img) {
	...
	void ProjectionRadial3::reorderParams(Image* img) {
	...
	ceres::CostFunction* ProjectionRadial3::getCostFunction() {
		return (new ceres::AutoDiffCostFunction<ProjectionRadial3,
			2,												// output residuals
			3,												// input 3D point	
			ProjectionRadial3::N_CAM_PARAMS,					// camera parameters
			ProjectionRadial3::N_IMG_PARAMS>(new ProjectionRadial3(_camera,_image,_X,_obs)));			// image parameters
	}

	// compute the residual of related 3D point projection and measured observation
	void ProjectionRadial3::computeResidual(double * res){
	...
	int ProjectionRadial3::numCamParams() {
		return ProjectionRadial3::N_CAM_PARAMS;
	}

	// number of used image parameters for specified image model
	int ProjectionRadial3::numImgParams() {
		return ProjectionRadial3::N_IMG_PARAMS;
	}
