...
3) Download and build boost-variant using VCPKG (e.g., run: <path/to/vcpkg.exe> install boost-variant --triplet x64-windows)
4) Use Cmake to create the project: 
4.1) Create build dir in the project root dir
4.2) Run cmake-gui in build dir and setup the variables:
...
4.3) [OPTIONAL] It is highly recommended to use some optimized BLAS and LAPACK library otherwise the code will be sllow. Please add:
...
4.4) To enable Matlab, check: USfM_BUILD_MEX = ON 
4.5) Run "Configure" (don't forget to select win64 option) and "Generate" 
5) Open the Visual Studio project, right clic on usfm root subroject in Solution Explorer and add in Properties > VC++ Directories > Library directories the path to compiler librearies (e.g. c:/Program Files (x86)/IntelSWTools/compilers_and_libraries_2017.4.210/windows/compiler/lib/intel64_win)
6) The Solution Explorer should contain:
